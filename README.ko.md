...
| # | 에이전트 행동 | 설명 |
|---|-------------|------|
| **1** | **컴포넌트 자동 인식** | DOM 구조를 분석하여 Navbar, Card Grid, Sidebar 등을 스스로 판단 |
| **2** | **디자인 개선 제안** | 간격 불균일, 정렬 미스 등을 능동적으로 감지하고 개선안 제안 |
| **3** | **반응형 자동 변환** | 원클릭 모바일 보기 &mdash; 375px 기준으로 레이아웃 자동 재배치 |
| **4** | **소스코드 리팩토링** | 시각적 변경을 실제 소스 파일에 매핑하여 정확한 diff 생성 |
| **5** | **자기 검증 루프** | 리팩토링 후 재렌더링 &rarr; 의도와 비교 &rarr; 불일치 시 자동 재수정 (최대 3회) |
...
```
Browser
├── URL 입력 ──> Puppeteer DOM 스캔 ──> AI 컴포넌트 인식
│                                              │
│                                              ▼
├── 비주얼 에디터 (iframe + 오버레이) <── 컴포넌트 스토어 (Zustand)
│   ├── 드래그 / 리사이즈 / 재배치             │
│   ├── 디자인 제안 [적용] [무시]              │
│   └── 모바일 보기 (AI 자동 배치)             │
│                                              │
...
| 엔드포인트 | 설명 |
|-----------|------|
| `POST /api/scan` | Puppeteer로 DOM 스캔 + 스크린샷 |
| `POST /api/detect` | AI 컴포넌트 자동 인식 |
| `POST /api/suggest` | AI 디자인 개선 제안 |
| `POST /api/responsive` | AI 반응형 레이아웃 변환 |
| `POST /api/refactor` | AI 소스코드 리팩토링 (diff 생성) |
| `POST /api/apply` | diff를 실제 소스 파일에 적용 |
| `POST /api/verify` | 자기 검증 + 자동 수정 루프 |
